...
async def fetch_openapi_spec(service: str, environment: str) -> Dict:
    ...
    if environment not in SERVICES[service]:
        raise ValueError(f"Unknown environment: {environment}. Available: {list(SERVICES[service].keys())}")
...
def create_generated_init_file() -> None:
    ...
    service_dirs = [d.name for d in GENERATED_DIR.iterdir() if d.is_dir() and not d.name.startswith('.')]
...
def main():
    ...
    parser.add_argument(
        "--service-name",
        help="Service name when using --file (required with --file)"
    )
