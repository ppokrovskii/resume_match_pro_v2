    ...
    def test_services_configuration(self):
        ...
        for service_name, environments in SERVICES.items():
            for env_name, url in environments.items():
                assert url.startswith("http"), f"Invalid URL for {service_name}/{env_name}: {url}"
                assert url.endswith("/openapi.json"), f"URL should end with /openapi.json: {url}"
...
''')
            ...
            with patch('pathlib.Path.mkdir'), \
                ...
                mock_open.return_value.__enter__.return_value.read.return_value = 'class TestModel(BaseModel):\n    name: str'
    ...
    def test_generate_models_for_service_subprocess_failure(self, mock_subprocess):
        ...
        mock_subprocess.side_effect = subprocess.CalledProcessError(1, "datamodel-codegen", stderr="Generation failed")
        ...
        try:
            with patch('generate_models.datetime') as mock_datetime:
                ...
                with open(temp_file, 'r') as f:
                    ...
                    assert "Auto-generated Pydantic models from Test Service API" in result
    ...
    def test_create_generated_init_file(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('generate_models.GENERATED_DIR', generated_dir):
                ...
                assert "Generated models from microservice OpenAPI specifications" in init_content
...
class TestIntegrationScenarios:
    ...
    def test_full_generation_workflow_simulation(self):
        ...
        mock_spec = {
            ...
            "paths": {
                "/documents": {
                    "get": {"operationId": "list_documents"}
                }
            },
            "components": {
                "schemas": {
